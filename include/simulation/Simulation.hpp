...
struct Simulation : std::enable_shared_from_this<Simulation> {
    ...
    // constructors
    Simulation()
        : galaxies(), octreeRoot(std::make_shared<Octree>(Vec3(0,0,0),1000.0f)), currentParticlesData(nullptr), currentFlattenedOctree(nullptr), stopFlag(false), galaxyFactory(nullptr) {Octree::root = octreeRoot;std::cout<<"Octree root width : "<<octreeRoot->width<<std::endl;}
    Simulation(scalar_t width)
        : galaxies(), octreeRoot(std::make_shared<Octree>(Vec3(0,0,0),width)), currentParticlesData(nullptr), currentFlattenedOctree(nullptr), stopFlag(false), galaxyFactory(nullptr) {Octree::root = octreeRoot;}
    Simulation(OctreePtr& octreeRoot_)
        : galaxies(), octreeRoot(octreeRoot_), currentParticlesData(nullptr), currentFlattenedOctree(nullptr), stopFlag(false), galaxyFactory(nullptr) {Octree::root = octreeRoot;}
    Simulation(Galaxies& galaxies_, OctreePtr& octreeRoot_)
        : galaxies(galaxies_), octreeRoot(octreeRoot_), currentParticlesData(nullptr), currentFlattenedOctree(nullptr), stopFlag(false), galaxyFactory(nullptr) {Octree::root = octreeRoot;}
